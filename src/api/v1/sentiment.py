...
from fastapi_cache.decorator import cache # <-- DITAMBAHKAN
...
@cache(expire=3600) # <-- DITAMBAHKAN: Cache hasil endpoint ini selama 1 jam
async def analyze_sentiment(
    ...
    try:
        ...
        print(f"CACHE MISS: Calling HuggingFace API for text: '{request.text}'") # Debugging
...
# 2. GET /api/requests
@router.get("/requests")
@cache(expire=60) # <-- DITAMBAHKAN: Cache daftar request selama 1 menit
...
@cache(expire=300) # <-- DITAMBAHKAN: Cache detail request selama 5 menit
